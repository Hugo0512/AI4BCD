...
train_data = CatsDogsDataset(patient_list_txtfile='./data/newdata_20240507/32/train5.txt',shuffle=True)
valid_data = CatsDogsDataset(patient_list_txtfile='./data/newdata_20240507/32/test5.txt',shuffle=True)
...
# loss function
criterion = nn.CrossEntropyLoss(weight=torch.tensor([3,1,1]).to(torch.float32).cuda())
# optimizer
...
for epoch in range(epochs):
    ...
    if (val_total_correct_number/val_sample_number)>best_accuracy:
        ...
        torch.save(model,'./data/newdata_20240507/32/BCD_best_wideresnet_5.pth')
    # print(
    #     f"Epoch : {epoch+1} - loss : {epoch_loss:.4f} - acc: {epoch_accuracy:.4f} - val_loss : {epoch_val_loss:.4f} - val_acc: {epoch_val_accuracy:.4f}\n"
    # )
torch.save(model,'./data/newdata_20240507/32/BCD_wideresnet_5.pth')
